...
- **`pepito-archive` must never be made public.** It holds `refs/pull/1/head`
  through `refs/pull/4/head`, which carry commits authored with a corporate
  email address and one blob with an absolute home path. Those refs are
  immutable from the client, so the only control available is visibility.
  This constraint has no expiry and no exception: it outlives this migration
  and applies to anyone who inherits the repository.
