...
Nitsan Avni). Read the source document
...
level. Here: `AGENTS.md` at the repo root, and its "Non-negotiable rules"
...
harness; no brand references). The user-level half is the agent's own
configuration (e.g. `~/.claude/CLAUDE.md` for Claude).
...
work. `AGENTS.md` deliberately keeps only the one-paragraph-per-boundary
...
written down in `.agents/docs/references/` (agent-facing) or `docs/`
...
`AGENTS.md`'s workflow section; and the written artifact to check that
...
committing on `main` is not trusted to prose — it
should be enforced by an agent hook or a CI gate, never by a rule alone.
(In the original Claude setup, a `PreToolUse` hook in
`~/.claude/settings.json` blocked commits on `main`/`master`.)
If the hook fires, the fix is a work branch, never a way around the hook.
Anything that must hold every single time belongs in a hook or a CI
...
checkboxes and the briefs' explicit verification steps, plus `AGENTS.md`'s
...
slop." Here: the four code smells that are not accepted, listed in `AGENTS.md`
